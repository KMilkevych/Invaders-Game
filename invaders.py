...
class InformationTable():
    ...
    def do_misc(self):
        """Renders texts containining how many lives and bigbullets player has left."""
...
class Level():
    ...
    """A Level class. Takes a layout as an argument, and can then generate a list of enemies at their corresponding positions based on different settings."""
...
class Decay:
    """A harmless decay object which is represented as a circle with same width and radius as the Blast it is created by"""
...
class Drawable:
    """A drawable class; basically a sprite that doesn't collide and can be drawn"""
...
def get_distance(p1, p2):
    """Gets distance between two points represented as lists or tuples"""
    sq = (p1[0]-p2[0])**2 + (p1[1] - p2[1])**2
    return math.sqrt(sq)

def load_content():

    """Loads sprites and other content from drive into RAM and saves in global dictionary"""
...
def generateLayout(columns = 7, rows = 6, d = 100, symmetric = True):
    ...
    """Generates a random symmetrical layout with a random amount of rows and columns and enemies based on the difficulty (d)"""
...
def doLevel(window, level, loading = True, lspeed = 1, pstart = "center", planetname = "PLANET"):
    ...
    #Creates and adds a tracker for those big bullets
...
def doLevelSelectionScreen(window):
    ...
    while running:
        ...
        #drawing the icons for the bigbullet and life counters        
